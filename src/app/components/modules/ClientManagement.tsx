...
import { PAGE_BG, ACCENT_DARK, ACCENT_ORANGE, ACCENT_ORANGE_HOVER, PANEL_BORDER, SOFT_BG, TILE_MINT, TILE_PEACH, TILE_BLUE, TILE_ROSE } from "../../../styles/moduleTheme";
    ...
    <div className={`min-h-screen ${PAGE_BG}`}>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
            <button onClick={() => loadData()}
              className={`flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-[#14110F] bg-white border ${PANEL_BORDER} rounded-full ${SOFT_BG} hover:bg-[#F4DCEF] transition`}>
            ...
            <button onClick={() => { setClientForm({ name: "", company: "", email: "", phone: "", phoneCountry: "IN", address: "", description: "", gstNumber: "", status: "active" }); setShowAddClient(true); }}
              className={`flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-white ${ACCENT_ORANGE} rounded-full ${ACCENT_ORANGE_HOVER} shadow-sm transition`}>
              <Plus className="w-4 h-4" /> Add Client
            </button>
            <button onClick={() => setShowCreateInvoice(true)}
              className={`flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-white ${ACCENT_DARK} rounded-full hover:bg-black shadow-sm transition`}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {[
            { label: "Active Clients",    value: activeClients,                              icon: Building2,  tile: TILE_MINT  },
            { label: "Total Invoiced",    value: `₹${totalInvoiced.toLocaleString("en-IN")}`, icon: FileText,   tile: TILE_BLUE  },
            { label: "Payments Received", value: `₹${totalPaid.toLocaleString("en-IN")}`,     icon: DollarSign, tile: TILE_PEACH },
            { label: "Outstanding",       value: `₹${totalOutstanding.toLocaleString("en-IN")}`, icon: TrendingUp, tile: TILE_ROSE },
          ].map(s => (
            <div key={s.label} className={`${s.tile} rounded-[24px] border ${PANEL_BORDER} p-3 sm:p-5 transition min-w-0`}>
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <span className="text-[10px] sm:text-xs font-semibold text-[#5b5648] uppercase tracking-wider truncate pr-1">{s.label}</span>
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-white/60 flex items-center justify-center shrink-0">
                  <s.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#14110F]" />
                </div>
              </div>
              <div className="text-lg sm:text-2xl font-bold text-[#14110F] truncate">{s.value}</div>
            </div>
          ))}
        </div>

        {/* Clients table */}
        <div className={`bg-white rounded-[28px] border ${PANEL_BORDER} shadow-sm overflow-hidden`}>
          <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 sm:px-6 py-4 border-b ${PANEL_BORDER}`}>
            <div>
              <h2 className="text-base font-bold text-[#14110F]">Clients</h2>
        ...
        {/* Recent invoices */}
        <div className={`bg-white rounded-[28px] border ${PANEL_BORDER} shadow-sm overflow-hidden`}>
          <div className={`flex items-center justify-between px-4 sm:px-6 py-4 border-b ${PANEL_BORDER}`}>
            <div>
              <h2 className="text-base font-bold text-[#14110F]">Recent Invoices</h2>
      ...
      {(showAddClient || showEditClient) && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white sm:rounded-[28px] border ${PANEL_BORDER} shadow-2xl w-full sm:max-w-lg h-full sm:h-auto max-h-full sm:max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-4 sm:p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2 pb-2 sm:pb-0">
                ...
                <button onClick={showEditClient ? handleEditClient : handleAddClient}
                  className={`flex-1 py-2.5 text-sm font-semibold text-white ${ACCENT_ORANGE} rounded-full ${ACCENT_ORANGE_HOVER} transition shadow-sm`}>
      ...
      {showDocuments && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white sm:rounded-[28px] border ${PANEL_BORDER} shadow-2xl w-full sm:max-w-2xl h-full sm:h-auto max-h-full sm:max-h-[90vh] flex flex-col overflow-hidden">
      ...
      {showCreateInvoice && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white sm:rounded-[28px] border ${PANEL_BORDER} shadow-2xl w-full sm:max-w-2xl h-full sm:h-auto max-h-full sm:max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100 sticky top-0 bg-white z-10">
              <div>
            ...
            <div className="p-4 sm:p-6 space-y-5">
              ...
              <div className="flex gap-3 pt-2 pb-2 sm:pb-0">
                ...
                <button onClick={handleCreateInvoice} className={`flex-1 py-2.5 text-sm font-semibold text-white ${ACCENT_DARK} rounded-full hover:bg-black transition shadow-sm`}>Create Invoice</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showClientInvoices && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white sm:rounded-[28px] border ${PANEL_BORDER} shadow-2xl w-full sm:max-w-5xl h-full sm:h-auto max-h-full sm:max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100">
              <div className="min-w-0">
      ...
      {showEditInvoice && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white sm:rounded-[28px] border ${PANEL_BORDER} shadow-2xl w-full sm:max-w-md h-full sm:h-auto max-h-full sm:max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 border-b border-slate-100 sticky top-0 bg-white z-10">
            ...
            <div className="p-4 sm:p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2 pb-2 sm:pb-0">
                ...
                <button onClick={handleUpdateInvoice} className={`flex-1 py-2.5 text-sm font-semibold text-white ${ACCENT_DARK} rounded-full hover:bg-black transition`}>Save Changes</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showPDFViewer && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center sm:p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white sm:rounded-[28px] border ${PANEL_BORDER} shadow-2xl w-full sm:max-w-4xl h-full sm:h-[90vh] flex flex-col overflow-hidden">
